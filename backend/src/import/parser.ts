...
function parseDateYmdSlash(s: string) {
  const m = s.match(/\b(20\d{2})\/(\d{2})\/(\d{2})\b/);
  if (!m) return null;
  return `${m[1]}-${m[2]}-${m[3]}`;
}

function parseDateCn(s: string) {
...
function parseTimeWindow(s: string) {
  const m = s.match(/\b(\d{2}:\d{2})-(\d{2}:\d{2})\b/);
  if (!m) return null;
  return `${m[1]}-${m[2]}`;
}

export function parseScreenshotText(raw: string) {
  ...
  if (text.includes('预约记录')) {
    ...
    for (const b of blocks) {
      ...
      const status =
        b.includes('未过期') ? 'VALID' : b.includes('已核销') ? 'REDEEMED' : b.includes('已过期') ? 'EXPIRED' : 'UNKNOWN';
